  ...
  return (
    <div className="h-full flex justify-center items-center bg-white">
      <h1 className="text-black text-4xl font-bold uppercase relative inline-block">
        <span id="before-te1" className="absolute top-0 left-0.5 w-full h-full bg-transparent" style={{ textShadow: "-2px 0 #49FC00", clipPath: "rect(24px, 550px, 90px, 0)" }} aria-hidden="true">{title}</span> {/* glitch::before */}
        {title}
        <span id="after-te1" className="absolute top-0 -left-0.5 w-full h-full bg-transparent" style={{ textShadow: "-2px 0 spin(#49FC00, 180)", clipPath: "rect(85px, 550px, 140px, 0)" }} aria-hidden="true">{title}</span> {/* glitch::after */}
      </h1>
    </div>
  )
}
